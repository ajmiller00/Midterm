    ...
    MongoClient.connect(url, { useUnifiedTopology: true }, function(err, db) {
		...
		coll.find(theQuery).toArray(function(err, items) {
		    ...
		    else 
		    {
        		...
        		res.write("<link rel='stylesheet' type = 'text/css' href= 'https://ajmiller00.github.io/Midterm/style.css'>");
        		...
        		res.write("<title>Rêve au Chocolat</title>");
        		res.write("</head><body><header><a href='index.html'><img src='logo-06.png' class='header'/></a>");
				...
				res.write("<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~Javascript~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->");
       			...
       			res.write("function abc() { if (document.getElementById('bur').style.display == 'none') { document.getElementById('burger').style.display = 'none';}}");
                ...
                // s0 = "";
                // s1 = "<select name='";
                // s2 = "' size='1' onchange = 'costCalc.call(this)' >";
                // s3 = "<option value='";
                // s4 = "'>";
                // s5 = "</option>";
                // s6 = "</select>";
                // res.write(`function select(name) { s = ${s0}; s = ${s1} + name + ${s2}; for (i = 0; i < 11; i++) {`);
                // res.write(`s += ${s3} + i + ${s4} + i + ${s5};} s += ${s6};document.writeln(s)}`);

                //res.write("HI");
            	
            	// res.write("function select(name) { s = ''; s = '<select name=&quot;' + name + '&quot; size=&quot;1&quot; onchange = &quot;costCalc.call(this)&quot; >';");
            	// res.write("for (i = 0; i < 11; i++) { s += '<option value=&quot;' + i + '&quot;>' + i + '</option>'; } s += '</select>'; document.writeln(s) } ");
				...
				res.write("<form class='giftFor' action='index.html' method='pos'><div class='gifts'><h1>Gifts</h1>");

				for (i=0; i<items.length; i++) {
					res.write("<div style ='margin-top: 0px;' class='gItem'><img src=\"" + items[i].img + "\" class=\"gift\">");
